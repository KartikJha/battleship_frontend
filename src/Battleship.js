...
const BattleshipBoard = ({ size, player1Board, player2Board }) => {
        ...
        {board.map((row, rowIndex) => (
          <div key={rowIndex} className="row">
            {row.map((cell, cellIndex) => (
              <div key={cellIndex} className={`cell ${cell.type}`}>
                {cell.hit && <div className="hit"></div>}
              </div>
            ))}
          </div>
        ))}
...
BattleshipBoard.propTypes = {
  ...
  player1Board: PropTypes.arrayOf(
    PropTypes.arrayOf(
      PropTypes.shape({
  ...
  player2Board: PropTypes.arrayOf(
    PropTypes.arrayOf(
      PropTypes.shape({
        type: PropTypes.string.isRequired,
        hit: PropTypes.bool.isRequired,
...
const BoardDemo = () => {
  const size = 10;
  const player1Board = Array(size).fill(Array(size).fill({ type: 'sea', hit: false }));
  const player2Board = Array(size).fill(Array(size).fill({ type: 'sea', hit: false }));

  return (
    <div className="App">
      <BattleshipBoard size={size} player1Board={player1Board} player2Board={player2Board} />
    </div>
  );
}
